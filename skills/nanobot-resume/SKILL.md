# /nanobot-resume — Resume Experiment#
...
```
/nanobot-resume                                  # List experiments, let user pick#
/nanobot-resume engineering/api-speed            # Resume specific experiment#
```
...
Resuming: engineering/api-speed#
  ...
  Last experiment: "added response caching" → KEPT (185ms)#
...
How would you like to continue?#
  1. Single iteration (/nanobot-run)   — I'll make one change and evaluate#
  2. Start a loop (/nanobot-loop)     — Autonomous with scheduled interval#
  3. Just show me the results    — I'll review and decide#
```

If the user picks loop, hand off to `/nanobot-loop` with the experiment pre-selected.#
If single, hand off to `/nanobot-run`.#
