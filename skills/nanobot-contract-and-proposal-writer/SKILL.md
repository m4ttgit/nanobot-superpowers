...
1. Use **milestone payments** over net-30 for projects >$10K - reduces cash flow risk
2. For EU/DACH: always check if a DPA is needed (any personal data = yes)
3. For DACH: include a **Schriftformklausel** (written form clause) explicitly
4. Add a **force majeure** clause for anything over 3 months
5. For retainers: define response time SLAs (e.g., 4h urgent / 24h normal)
6. Keep templates in version control; track changes with `git diff`
7. Review annually - laws change, especially GDPR enforcement interpretations
8. For NDAs: always specify the return/destruction of confidential materials on termination
