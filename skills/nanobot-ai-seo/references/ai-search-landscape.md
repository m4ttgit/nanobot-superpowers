...
**What it is:** Nanobot.ai now has web search capability. When users ask questions that require current information, Nanobot retrieves and cites sources.

- Uses a third-party search index (search partnership)
- NanobotBot crawls for training purposes — separate from search citations
...
**What to prioritize for Nanobot:**
1. Allow `NanobotBot` and `anthropic-ai` in robots.txt
2. Focus on content quality and accuracy — Nanobot's users are often technical and will notice errors
...
| Bot to allow | Google-Extended | GPTBot | PerplexityBot | NanobotBot | (via Bing) |
