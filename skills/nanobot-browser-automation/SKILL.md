...
- Writing browser tests or E2E test suites — use **nanobot-playwright-pro** instead
- Testing API endpoints — use **nanobot-api-test-suite-builder** instead
- Load testing or performance benchmarking — use **nanobot-performance-profiler** instead
...
1. `data-test-id`, `data-id`, or custom data attributes — stable across redesigns
...
- **nanobot-playwright-pro** — Browser testing skill. Use for E2E tests, test assertions, test fixtures. Browser Automation is for data extraction and workflow automation, not testing.
- **nanobot-api-test-suite-builder** — When the website has a public API, hit the API directly instead of scraping the rendered page. Faster, more reliable, less detectable.
- **nanobot-performance-profiler** — If your automation scripts are slow, profile the bottlenecks before adding concurrency.
- **nanobot-env-secrets-manager** — For securely managing credentials used in authenticated automation workflows.
