...
def print_report(competitors: list, insights: list):
    ...
    print("\n  Headers: Handle, Followers, Posts/wk, Eng Rate")
    print("  " + "─"*20 + " " + "─"*10 + " " + "─"*10 + " " + "─"*10)
    for c in competitors:
        er = calculate_engagement_rate(c)
        print("  {handle:<20} {followers:>10,} {posts:>10.0f} {er:>9.2f}%".format(
            handle=c.handle, followers=c.followers, posts=c.posts_per_week, er=er))
    ...
    if insights:
        ...
        for i in sorted_insights:
            ...
            print("  {icon} [{cat}] {find}".format(icon=icon, cat=i.category, find=i.finding))
            print("     → {opp}".format(opp=i.opportunity))
            print()
  ...
  [{"handle": "@user1", "followers": 50000, "posts_per_week": 14, ...}]
    ...
    if args.import_file:
        ...
        if all(c.followers == 0 for c in competitors):
            print("\n  ℹ️  Handles registered: {}".format(', '.join(c.handle for c in competitors)))
            ...
            print("  3. Run: {} --import data.json".format(sys.argv[0]))
            print("\n  Example JSON:")
