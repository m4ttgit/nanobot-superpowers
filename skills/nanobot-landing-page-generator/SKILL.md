...
2. **Analyze brand voice** (recommended) — if the user has existing brand content (website copy, blog posts, marketing materials), run it through `nanobot-content-production/scripts/brand_voice_analyzer.py` to get a voice profile (formality, tone, perspective). Use the profile to inform design style and copy framework selection:
...
Map over a `features` array with `{ title, description, image, badge }`. Toggle layout direction with `i % 2 === 1 ? "lg:flex-row-reverse" : ""`. Use `<Image>` with explicit `width`/`height` and `rounded-2xl shadow-xl`. Container: `max-w-6xl` container.#
...
- **Brand Voice Analyzer** (`nanobot-content-production/scripts/brand_voice_analyzer.py`) — Run before generation to establish voice profile and ensure copy consistency#
- **UI Design System** (`nanobot-ui-design-system/`) — Generate design tokens from brand color before building the page#
- **Competitive Teardown** (`nanobot-competitive-teardown/`) — Competitive positioning informs landing page messaging and differentiation#
